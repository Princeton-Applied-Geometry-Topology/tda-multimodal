...
class Qwen(LLM):
    '''Qwen model implementation using TransformerLens'''

    def __init__(
        self,
        model_name: str = None,
        weights_path: str = 'Qwen/Qwen2.5-7B', # Default to Qwen 2.5 7B
        **kwargs
    ):
        self.model_name = model_name
        super().__init__(weights_path=weights_path, model_name=model_name, **kwargs)
    ...
    def _load_and_prepare_ablation_coords(self):
        ...
        # Construct path based on the specific model name derived from weights_path
        aie_scores_path = Path(f'aie_scores/abstractive/{self.model_name}.pt')
    ...
    def _collate_metric_results(self, raw_results: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for hook_name, scores in raw_results.items():
            ...
            if len(parts) > 1 and parts[0] == 'blocks' and parts[1].isdigit():
                 ...
                 # Handle potential unexpected hook names if necessary
                 print(f"Warning: Could not parse layer index from hook name: {hook_name}")
                 final_results[hook_name] = scores # Keep original name if parsing fails
    ...
    def forward_with_metrics(
        ...
        with torch.no_grad():
            ...
            for metric_name, metric_fn in metrics_to_compute.items():
                ...
                # Using 'hook_resid_post' which is generally available after MLP and Add & Norm
                for layer_idx in range(self.model.cfg.n_layers):
