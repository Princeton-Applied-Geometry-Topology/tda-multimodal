...
class Qwen2VLDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        residual = self.id_pre_res(hidden_states)
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            hidden_states=self.pre_attn(hidden_states),
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_value=past_key_value,
            output_attentions=output_attentions,
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        for i, layer in enumerate(self.model.layers):
            modified_layer = Qwen2VLDecoderLayer(config, layer.self_attn.layer_idx)
            modified_layer.self_attn = Qwen2VLAttention(config, layer_idx=layer.self_attn.layer_idx)
            modified_layer.id_pre_res = nn.Identity()
            modified_layer.pre_attn = nn.Identity()
            self.model.layers[i] = modified_layer
