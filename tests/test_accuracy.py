...
class TestAccuracyComputation(unittest.TestCase):
    def setUp(self):
    ...
    def test_accuracy_on_all_data(self):
        """
        Tests accuracy computation on all JSON files in the data directory.
        For each example, it verifies that perfect predictions yield an accuracy
        of 1.0 and incorrect predictions yield 0.0.
        """
        ...
        for subdir in data_subdirs:
            ...
            for task_file in data_dir.glob('*.json'):
                print(f"Testing accuracy for task: {task_file.name}")
                ...
                if not isinstance(examples, list) or not examples:
                    self.skipTest(f"Skipping {task_file} due to unsupported format or empty list.")
                    continue
                ...
                for i, example in enumerate(examples):
                    with self.subTest(task=task_file.name, example_index=i):
                        ...
                        # Test perfect prediction (should yield 1.0 accuracy)
                        accuracy_perfect = self.compute_accuracy(predicted_answer, ground_truth, mode='all')
                        self.assertEqual(accuracy_perfect, 1.0, 
                            f"Perfect prediction should yield 1.0 accuracy for {task_file.name} example {i}. "
                            f"Got: {accuracy_perfect}, Predicted: '{predicted_answer}', Ground truth: '{ground_truth}'")

                        # Test incorrect prediction (should yield 0.0 accuracy)
                        wrong_prediction = "WRONG_ANSWER"
                        accuracy_wrong = self.compute_accuracy(wrong_prediction, ground_truth, mode='all')
                        self.assertEqual(accuracy_wrong, 0.0,
                            f"Wrong prediction should yield 0.0 accuracy for {task_file.name} example {i}. "
